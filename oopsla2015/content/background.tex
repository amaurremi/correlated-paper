...
A path through the control-flow supergraph is \textit{valid} if it follows the usual stack-based calling 
...
\commentout{
Note that $R_f$ constructs pairs of data-flow facts so that
\begin{itemize}
...
\begin{example}\label{ex:flowfun}
  ...
  %and data-flow function $f=\lambda S\,.\,S\setminus\{v\}\cup\{u\}$ looks as follows:
  $
...
Using the flow functions, the IFDS algorithm computes the interprocedurally \textit{realizable} paths from the start to the rest of the exploded graph's nodes. A realizable path is a valid path in the exploded supergraph that starts with the entry node $\startmain$.
...
exploded supergraph edges $E^\sharp$ instead of explicit data-flow functions $M_F$. This is convenient because
...
$f: N^* \times D \times N^* \to 2^D$. Given a (non-exploded) supergraph edge $n \to n'$ and a data-flow fact $d$,
...
%\mtodo{Do we leave the start-to-return edge out as a special case of intraprocedural edge?}
}
...
\begin{align*}
    R_t =& \{ \microedge{\mathbf0}{\mathbf0}{\lambda l.l} \} \cup
    \{\microedge{\mathbf0}{d_j}{\lambda l.t(\top_{\textsf{Env}})(d_j)} \mid d_j \in D\}\\
    &\cup
    \{\microedge{d_i}{d_j}{\lambda l.t(\top_{\textsf{Env}}[d_i\to l])(d_j)} \mid d_i, d_j \in D\}.
\end{align*}
}
%Let $\top_{\textsf{Env}} = \lambda d.\top$ be the environment that maps every element of $D$ to $\top$.
%\ftodo{
...
The overall idea behind computing the lattice elements $l_{n,\,d}$ is the following. For each interprocedurally realizable path
\[
  p=\left[(\startmain,\,\mathbf0),\,(n_1,\,d_1),\,\dots,\,(n_k,\,d_k)\right]
\]
...
\begin{align}
  f_p=\textsf{EdgeFn}((n_{k-1},\,d_{k-1}),\,(n_k,\,d_k))\,\circ\ldots\circ
      \textsf{EdgeFn}(\startmain,\,\mathbf0),\,(n_1,\,d_1))\,.
\end{align}
...
%  \otodo{See Example 4 on the previous page.}
%  \otodo{Now done in Example 5.}
