...
% main method edges 
    \path[->] (st_main) edge (asgn_a);
    \path[->] (asgn_a) edge (call_foo);
    \path[->] (call_foo) edge (return_foo);
    \path[->] (return_foo) edge (call_bar);
    \path[->] (call_bar) edge (return_bar);
    \path[->] (return_bar) edge (end_main);
...
%inter-procedural edges
  \path[->,dashed] (call_foo) edge[out=20,in=190] (st_a_foo);
  \path[->,dashed] (call_foo) edge[color=lightsalmonpink,out=-20,in=135] (st_b_foo);
  \path[->,dashed] (call_bar) edge[out=25,in=220] (st_a_bar);
  \path[->,dashed] (call_bar) edge[color=lightsalmonpink,out=18,in=160] (st_b_bar);
  \path[->,dashed] (end_a_foo) edge[out=220,in=15] (return_foo);
  ...
  \path[->,dashed] (end_b_bar) edge[color=lightsalmonpink,out=200,in=-40] (return_bar);
...
\end{tikzpicture}
\caption{An example supergraph for Listing~\ref{list:ccexample}}%
