...
\begin{tikzpicture}
    \node [problem]
      (ifds) {IFDS problem};
    \node [problem]
      (equiv) [above right=0.5cm and .7\dist of ifds.east] {Equivalence-IDE problem};
    \node [problem]
    ...
    \path[->] (equiv) edge node[above,font=\scriptsize]{$\resultIDE$} (equivres);
    \path[->] (ccide) edge node[above,font=\scriptsize]{$\resultIDE$} (ccres);
    \path[->,dashed] (ifds) edge[out=-3,in=183] node[above,font=\scriptsize]{$\resultIFDS$} (ifdsres);
\end{tikzpicture}
