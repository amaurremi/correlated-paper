...
\end{tikzpicture}
\vspace{-27mm}
\caption[An example program demonstrating correlated-call edge functions on the $\mathbf0$-node path for Listing~\ref{list:ccexample}]{An example program demonstrating correlated-call edge functions on the $\mathbf0$-node path for Listing~\ref{list:ccexample}. All non-labeled edges are implicitly labeled with identity functions $\id$. The variable $r$ denotes the return value of the \texttt{A.foo} method.}%
