...
To evaluate the practicality of our work, we applied two variants of the IFDS taint analysis
...
calls. Unfortunately, for the specific combination of this IFDS client analysis and these benchmark
programs, the precise tracking of data flow in the presence of correlated calls does not impact precision. 
However, the good news is that the running times of the two analyses are roughly the same, ranging from 
a few seconds on the smaller SPEC programs to about two hours on \texttt{javac} in each cases (see the Appendix
for more details on these results).  This suggests that the overhead of tracking correlated calls is manageable,
