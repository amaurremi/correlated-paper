...
\begin{enumerate}[(i)]
	\item an equivalent IDE taint analysis obtained using $\transEq$, and
	\item an IDE taint analysis obtained using   $\transCC_\rcc$ that avoids imprecision due to correlated method
calls.
\end{enumerate}
The equivalence analysis is there for two reasons: 
\begin{enumerate}[(i)]
	\item  to explain how a correlated-calls-IDE problem can be derived from an IDE problem that has the same meaning as the original IFDS problem; 
	\item (ii) to provide a base line against which to compare the efficiency of the correlated-calls analysis.
\end{enumerate}
