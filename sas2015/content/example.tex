...
%that we will use to illustrate
%how our technique improves the precision of a simple taint analysis.
The program
...
%To keep the 
%example small and self-contained, it is 
...
%via calls \code{System.out.println()} such as the one in method \code{B.bar()} 
%on line~\ref{line:Bbar} 
...
either an \code{A}-object or a \code{B}-object%
%on line~\ref{line:createObject}
. The 
...
?
%To determine whether such a flow of tainted data is possible,
...
since the variable \code{a} may point to either an \code{A}-object or a \code{B}-object, 
