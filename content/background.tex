...
Specifically,
the input is generally provided as a function $f: (N^* \times D) \times (N^* \times D) \to F$,
where $F$ is the set of representations of micro-functions from $L$ to $L$.
Given an exploded supergraph edge $e=(n,\,d) \to (n',\,d')$,
$f((n,\,d),\,(n',\,d'))$ returns the micro-function that appears on the exploded supergraph edge $e$.
Like in the IFDS algorithm, the function $f$ can be split up into separate functions that handle the cases when $n\to n'$ is an intraprocedural edge, a call-to-start edge, or an end-to-return edge.
