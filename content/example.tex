...
shared by both collections and iterators over them. The \code{toArray} method of this trait
creates an array and copies the contents of the collection or iterator into it:
\inputMinted{scala}{collectionsExample.scala}
\otodo{Marianna, can you figure out how to get LaTeX to syntax-highlight this?} 
When \code{this} is an iterator rather than a collection, the call to \code{this.size}
extracts all elements of the iterator to count them. At the call to \code{copyToArray},
...
The implementation of \code{TraversableOnce.toArray} is actually correct because the above
code is guarded with a test:
\inputMinted{scala}{isTraversable.scala}
