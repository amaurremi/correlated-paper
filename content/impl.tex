...
\begin{example}
  ...
  Returning a secret value in method \code{A.foo} creates a ``diagonal'' edge from the $\Lambda$-fact to the secret fact $\psi$. 
...
On the end-return edge from \code{A.foo} to \code{main}, the set of types for the receiver \code{a} is restricted by the micro function $\lambda m\,.\,m[\code a\to m(\code a)\cap\{\code A\}]$ on the edge corresponding to the assignment of the return value $\psi$ to \code v.
...
\subsection{Converting IDE Results to IFDS Results}\label{sec:ideToIfds}\mtodo{I changed the formalisms in this section. Please see if you agree to these changes.}
...
infeasible. Therefore, a result $\rho=\resultIDE(\transCC S(P))$ of the correlated-calls IDE analysis is
