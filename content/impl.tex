...
$N \to D$.
\mtodo{Should it be $N^*$ and $\resultIDE(Q)$?}
Similarly, for an IFDS problem $Q$, the result $\resultIDE$
...
Therefore, a result $\rho$ of the equivalence IDE analysis
is converted to an IFDS result as follows:
$\backEq(\rho) = \lambda n.\{d\mid\rho(n) = (d,\top)\}$.
\mtodo{For the proofs, instead of talking about the transformation for an equivalence-IDE problem,
we should talk about the equivalence transformation for any IDE problem. So we should define
$\backEq(\rho) = \lambda n.\{d\mid\rho(n) = (d,\ell),\,\ell\ne\topcc\}$.}
