...
\otodo{we agreed to move this figure to the initial example section}
  ...
  \otodo{Can we combine the IDE Figure\ref{fig:cc_edgefn_example} with the IFDS Figure 4? It should be the same figure, just that Figure\ref{fig:cc_edgefn_example} has labels on the edges. We can initially tell the reader to ignore the labels until later.}
...
\todo{What does the notation $N^*$ and $N^\#$ mean? What is the difference between the two?}
...
in definition \ref{def:microfn} can be represented in this form. \otodo{Say
    how application, comparison, composition, and meet are implemented. Decide
    how much detail and how much proof to give. Also need to introduce normalization.}
...
\begin{lemma}[Precision]\label{lem:subsetifds}
    ...
    \end{equation}
\end{lemma}
\otodo{We agreed to move the proofs to the appendix.}
\begin{proof}
