...
\end{equation}
%\mtodo{Another change was that Ondrej defined $\backEq$ as $\lambda n.\{d\mid\rho(n)(d)=\top\}$
...
\begin{restatable}{lemma}{efficient}\label{lem:efficient}
  ...
  correlated-call receivers $R$ and the number of run-time types $T$ are considered constant.
\end{restatable}

From this lemma, we conclude that the correlated-calls analysis runs in time
$O(ED^3RT)$. In other words, the asymptotic cost of the added precision of
the correlated-calls analysis is increased by a factor of $O(RT)$ compared
