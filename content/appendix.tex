...
\begin{proof}
  ...
  where $r_1,\,\dots,r_l\in R$ are the local variables in the called method.
    ...
    The fact that $\mpd(r)\ne\top_T$ means that there exists a realizable path corresponding to the fact $d$ along path $p$, and, by definition, $d$ must be contained in $M_F(p)(\varnothing)$.\mtodo{Is this hand-wavy?}
\end{proof}
