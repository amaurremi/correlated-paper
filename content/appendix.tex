\section*{Appendix}
In this appendix we present the proofs to the Lemmas introduced in Section~\ref{sec:ccdatastr}.
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lem:subsetifds}}]
  ...
  \begin{align*}
    \backCC\left(\resultIDE(\transCC_R(P))\right)(n)&=\{(n',\,D_n'^\Subset(\resultIDE(\transCC_R(P))))\ |\ n\in N^*\}(n)\\
      &=D_n^\Subset(\resultIDE(\transCC_R(P)))\\
      &\subseteq\mvp F(n)\\
      &=\resultIFDS(P)(n)\,.\qedhere
  \end{align*}
\end{proof}
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lem:sound}}] Let $\rho=\resultIDE(\transCC_R(P))$. Then
  \begin{align*}
    \backCC(\rho)(n)
    &=D_n^\Subset(\rho)\\
    &=\left\{d'\ |\ 
...
\end{lemma}
\begin{proof}
 Recall from Section~\ref{sec:ideToIfds} that the result of an IDE analysis maps a lattice element to each node in the exploded supergraph. Specifically, for an IDE problem $Q$, the result $\resultIDE(Q):\ N^\#\to L$ maps nodes of the exploded supergraph to lattice elements:
\begin{equation}\label{eq:ideresult}
  \resultIDE(Q)=\{((n,\,d),\,\mvp{\textsf{Env}}(n,\,d))\ |\ n\in N^*,\,d\in D\}\,.
\end{equation}
Furthermore, recall from Section~\ref{sec:bgide} that
...
Finally, we present the proof for Lemma~\ref{lem:onlycorrec} which states that a correlated-calls analysis that considers all receivers computes the same result as an analysis that considers only correlated-call receivers.
...
Since, according to Lemma~\ref{lem:ccrectop}, $\resultIDE(\transCC_{\{r\}}(P))(n,\,d)(r)$ can only be equal to $\top_T$ when $r\in\rcc$, we can conclude that
  \begin{align*}
    D_n^\Subset&(\resultIDE(\transCC_R(P))))\\
    &=\left\{d\ |\ d\in\mvp F(n)\,\wedge\,\forall\bm{r\in\rcc}:\ \resultIDE(\transCC_{\{r\}}(P))(n,\,d)(r)\ne\top_T\right\}\\
    &=D_n^\Subset(\resultIDE(\transCC_\rcc(P)))).
  \end{align*}
  
  Therefore,
  \begin{align*}
    \backCC(\resultIDE\left(\transCC_R(P)\right))
    &=\left\{(n,\,D_n^\Subset(\resultIDE(\transCC_\rcc(P))))\ |\ n\in N^*\right\}\\
    &=\backCC(\resultIDE\left(\transCC_\rcc(P)\right))\,.\qedhere
  \end{align*}
\end{proof}

\section*{Representation of Micro Functions}
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lem:normeq}}]
...
\begin{proof}[\textbf{Proof of Lemma~\ref{lem:normuneq}}]
